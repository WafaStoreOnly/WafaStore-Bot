...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!custom_id) {
      return res.status(200).json({ type: 4, data: { content: "❌ No action bos!", flags: 64 } });
    }
    ...
    let msg = '';
    if (newStatus === 'proses') msg = `⚡ ORDER #${orderId.slice(-6)} DIPROSES! Buyer udah dapet notif "sedang di proses 🥰"`;
    if (newStatus === 'selesai') msg = `✅ ORDER #${orderId.slice(-6)} SELESAI! Buyer udah dapet notif "sudah diterima transaksi done ✅ 📌 ticket akan terhapus otomatis dalam 10 menit + Terimakasih WafaStoreOnly🥰🙏"`;
    if (newStatus === 'batal') msg = `❌ ORDER #${orderId.slice(-6)} DIBATALKAN!`;
    return res.status(200).json({ type: 4, data: { content: msg, flags: 64 } });
  } catch (e) {
    return res.status(200).json({ type: 4, data: { content: "❌ Error: " + e.message, flags: 64 } });
  }
}
